...
initialDronePosition = Position 320 240 

initialDroneVelocity :: Velocity
initialDroneVelocity  = Velocity 0 0 

initialDroneAcceleration :: Acceleration
initialDroneAcceleration = Acceleration 0 0 
...
initialTargetPosition = Position 10 10  

initialTargetVelocity :: Velocity
initialTargetVelocity = Velocity 1 1 

initialTarget :: Entity
initialTarget = Entity initialTargetPosition initialTargetVelocity (Acceleration 0 0) blue
...
addNoiseToEntity (Entity(Position x y) vel acc color) noise = 
    Entity (Position (x + realToFrac noise) (y + realToFrac noise)) vel acc color

catmullRom :: Position -> Position -> Position -> Position -> Float -> Position
catmullRom (Position x0 y0) (Position x1 y1) (Position x2 y2) (Position x3 y3) t =
    Position x y
    where 
        t' = realToFrac t 
